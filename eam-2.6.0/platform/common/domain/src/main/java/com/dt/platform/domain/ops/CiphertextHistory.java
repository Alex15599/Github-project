...
import com.dt.platform.constants.db.OpsTables.OPS_CIPHERTEXT_HISTORY;
...
@Table(name = "ops_ciphertext_history")
public class CiphertextHistory extends Entity {

	private static final long serialVersionUID = 1L;

	public static final DBTable TABLE =OPS_CIPHERTEXT_HISTORY.$TABLE;
	...
	*/
	 ...
	 * 加密内容：加密内容
	*/
	@ApiModelProperty(required = false,value="加密内容" , notes = "加密内容")
	...
	*/
	@Transient
	public CiphertextHistory setDeleted(Boolean deletedBool) {
		if(deletedBool==null) {
			this.deleted=null;
		} else {
	...
	*/
	 ...
	 * 将自己转换成指定类型的PO
	 * @param poType  PO类型
	 * @return CiphertextHistory , 转换好的 CiphertextHistory 对象
	*/
	@Transient
	 ...
	 * @return CiphertextHistory , 转换好的 PoJo 对象
	*/
	@Transient
	...
	*/
	@Transient
	public CiphertextHistory clone(boolean deep) {
		return EntityContext.clone(CiphertextHistory.class,this,deep);
	}

	/**
	 * 将 Map 转换成 CiphertextHistory
	 * @param ciphertextHistoryMap 包含实体信息的 Map 对象
	 * @return CiphertextHistory , 转换好的的 CiphertextHistory 对象
	*/
	@Transient
	public static CiphertextHistory createFrom(Map<String,Object> ciphertextHistoryMap) {
		if(ciphertextHistoryMap==null) return null;
		CiphertextHistory po = create();
		EntityContext.copyProperties(po,ciphertextHistoryMap);
		po.clearModifies();
		return po;
	}

	/**
	 * 将 Pojo 转换成 CiphertextHistory
	 * @param pojo 包含实体信息的 Pojo 对象
	 * @return CiphertextHistory , 转换好的的 CiphertextHistory 对象
	*/
	@Transient
	public static CiphertextHistory createFrom(Object pojo) {
	...
	/**
	 * 创建一个 CiphertextHistory，等同于 new
	 * @return CiphertextHistory 对象
	*/
	@Transient
	public static CiphertextHistory create() {
		return new com.dt.platform.domain.ops.meta.CiphertextHistoryMeta.$$proxy$$();
	}
}