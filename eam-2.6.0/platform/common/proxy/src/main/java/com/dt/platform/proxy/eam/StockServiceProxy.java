...
public interface StockServiceProxy {
    ...
    Result insert(@RequestParam(name = "stockVO") StockVO stockVO);
    ...
    Result update(@RequestParam(name = "stockVO") StockVO stockVO);

    /**
     * 更新资产库存
     */
    @RequestMapping(StockServiceProxy.SAVE)
    Result save(@RequestParam(name = "stockVO") StockVO stockVO);

    /**
     * 获取资产库存
     */
    @RequestMapping(StockServiceProxy.GET_BY_ID)
    Result<Stock> getById(@RequestParam(name = "id") String id);

    /**
     * 批量删除资产库存
     */
    @RequestMapping(StockServiceProxy.GET_BY_IDS)
    Result<List<Stock>> getByIds(@RequestParam(name = "ids") List<String> ids);

    /**
     * 查询资产库存
     */
    @RequestMapping(StockServiceProxy.QUERY_LIST)
    Result<List<Stock>> queryList(@RequestParam(name = "sample") StockVO sample);

    /**
     * 分页查询资产库存
     */
    @RequestMapping(StockServiceProxy.QUERY_PAGED_LIST)
    Result<PagedList<Stock>> queryPagedList(@RequestParam(name = "sample") StockVO sample);
