...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-cust/cust-clps-demo";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'title', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('title') , templet: function (d) { return templet('title',d.title,d);}  }
					,{ field: 'user', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('user') , templet: function (d) { return templet('user',d.user,d);}  }
					,{ field: 'org', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('org') , templet: function (d) { return templet('org',d.org,d);}  }
					,{ field: 'status', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('status'), templet:function (d){ return templet('status',fox.getEnumText(SELECT_STATUS_DATA,d.status,'','status'),d);}}
					,{ field: 'content', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('content') , templet: function (d) { return templet('content',d.content,d);}  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.user={ inputType:"button",value: $("#user").val() ,fuzzy: true,splitValue:false,valuePrefix:"",valueSuffix:"" };
		value.org={ inputType:"button",value: $("#org").val() ,fuzzy: true,splitValue:false,valuePrefix:"",valueSuffix:"" };
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('cust-clps-demo-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('cust-clps-demo-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'CLPS测试'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'CLPS测试'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('cust-clps-demo-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('cust-clps-demo-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('cust-clps-demo-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'CLPS测试'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('cust-clps-demo-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('cust-clps-demo-form-data', data);
		var area=admin.getTempData('cust-clps-demo-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('CLPS测试');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"cust-clps-demo-form-data-win",
			content: '/business/customer/clps_demo/clps_demo_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
