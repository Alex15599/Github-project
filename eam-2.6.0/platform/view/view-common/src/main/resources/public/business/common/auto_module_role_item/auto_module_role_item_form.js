...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('sys-auto-module-role-item-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('sys-auto-module-role-item-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'sys-auto-module-role-item-form-data-win');
			if(area==null) return;
			admin.putTempData('sys-auto-module-role-item-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("moduleRoleId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('sys-auto-module-role-item-form-data');
		}
		...
		if (hasData) {
			...
			//设置  模块 设置下拉框勾选
			fox.setSelectValue4Enum("#moduleRoleId",formData.moduleRoleId,SELECT_MODULEROLEID_DATA);
	...
	function getFormData() {
		...
		//获取 模块 下拉框的值
		data["moduleRoleId"]=fox.getSelectedValue("moduleRoleId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('sys-auto-module-role-item-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('sys-auto-module-role-item-form-data-win',this); });
