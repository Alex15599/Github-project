...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-common/sys-auto-module-role";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'module', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('模块'), templet:function (d){ return templet('module',fox.getEnumText(SELECT_MODULE_DATA,d.module,'','module'),d);}}
	...
	function bindSearchEvent() {
		...
		$('#search-button').click(function () {
			...
			var URL="";
			top.layer.confirm(fox.translate('确定服务降级吗？'), function (i) {
				top.layer.close(i);

				var URL=moduleURL+"/permission-downgrade"
				admin.post(URL, {}, function (data) {
					if(data.success) {
					} else {
					}
					fox.showMessage(data);
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('sys-auto-module-role-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
		...
		function openCreateFrom() {
			//设置新增是初始化数据
			var data={};
			admin.putTempData('sys-auto-module-role-form-data-form-action', "create",true);
			showEditForm(data);
		};
		...
		function batchDelete(selected) {
			...
			if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'模块角色'+"!"));
				return;
			}
			//调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'模块角色'+'吗？'), function (i) {
				top.layer.close(i);
	...
	function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('sys-auto-module-role-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('sys-auto-module-role-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('sys-auto-module-role-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'模块角色'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('sys-auto-module-role-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('sys-auto-module-role-form-data', data);
		var area=admin.getTempData('sys-auto-module-role-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('模块角色');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"sys-auto-module-role-form-data-win",
			content: '/business/common/auto_module_role/auto_module_role_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
