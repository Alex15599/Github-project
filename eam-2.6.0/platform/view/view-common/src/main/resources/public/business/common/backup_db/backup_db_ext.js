...
layui.define(['form', 'table', 'util', 'settings', 'admin', 'upload','foxnic','xmSelect','laydate','foxnicUpload','dropdown'],function () {
    ...
    var list={
        ...
        afterQuery : function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                if(data[i].status=="valid") {
                    ...
                    fox.disableButton($('.down-file').filter("[data-id='" + data[i].id + "']"), true);
                }
            }
        },
        /**
        ...
        executeBackup:function(selected,obj){
            ...
            var btn=  $('#execute-backup');
            var api=moduleURL+"/execute-backup"
            admin.post(api, {}, function (r) {
                if (r.success) {
                    top.layer.msg("执行成功", {time: 1000});
                } else {
                    fox.showMessage(r);
                }
