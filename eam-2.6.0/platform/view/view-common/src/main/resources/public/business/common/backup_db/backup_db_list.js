...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-common/sys-backup-db";
	var dataTable=null;
	var sort=null;
	/**
      * 入口函数，初始化
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'fileRoute', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('备份文件') , templet: function (d) { return templet('fileRoute',d.fileRoute,d);}  }
					,{ field: 'fileSize', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('大小') , templet: function (d) { return templet('fileSize',d.fileSize,d);}  }
					...
					,{ field: 'invalidTime', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('失效日期') ,templet: function (d) { return templet('invalidTime',fox.dateFormat(d.invalidTime,"yyyy-MM-dd HH:mm:ss"),d); }  }
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('sys-backup-db-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
					batchDelete(selected);
					break;
				case 'tool-execute-backup':
					window.pageExt.list.executeBackup && window.pageExt.list.executeBackup(selected,obj);
					break;
				case 'refresh-data':
					refreshTableData();
					break;
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('sys-backup-db-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'备份记录'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'备份记录'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('sys-backup-db-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('sys-backup-db-form-data-form-action', "edit",true);
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('sys-backup-db-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'备份记录'+'吗？'), function (i) {
					top.layer.close(i);
			...
			else if (layEvent === 'down-file') { // 下载
				window.pageExt.list.downFile(data,this);
			}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('sys-backup-db-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('sys-backup-db-form-data', data);
		var area=admin.getTempData('sys-backup-db-form-area');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"sys-backup-db-form-data-win",
			content: '/business/common/backup_db/backup_db_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
