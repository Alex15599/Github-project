...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('oa-vehicle-info-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('oa-vehicle-info-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'oa-vehicle-info-form-data-win');
			if(area==null) return;
			admin.putTempData('oa-vehicle-info-form-area', area);
	...
	function renderFormFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("rescueDueDate",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#scrapTime',
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('oa-vehicle-info-form-data');
		}
		...
		if (hasData) {
			...
			//设置 抢险到期 显示复选框勾选
			if(formData["rescueDueDate"]) {
				$("#rescueDueDate").val(fox.dateFormat(formData["rescueDueDate"],"yyyy-MM-dd"));
			}
			//设置 报废时间 显示复选框勾选
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('oa-vehicle-info-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('oa-vehicle-info-form-data-win',this); });
