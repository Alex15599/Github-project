...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-workorder/wo-server-info";
	var dataTable=null;
	var sort=null;
	/**
      * 入口函数，初始化
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'memory', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('内存(G)') , templet: function (d) { return templet('memory',d.memory,d);}  }
					,{ field: 'spaceSize', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('空间(G)') , templet: function (d) { return templet('spaceSize',d.spaceSize,d);}  }
					,{ field: 'serverNumber', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('数量') , templet: function (d) { return templet('serverNumber',d.serverNumber,d);}  }
					,{ field: 'userlist', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('用户列表') , templet: function (d) { return templet('userlist',d.userlist,d);}  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.serverType={ inputType:"radio_box", value: getSelectedValue("#serverType","value"), label:getSelectedValue("#serverType","nameStr") };
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("serverType",data.arr,data.change,data.isAdd);
				},1);
			},
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('wo-server-info-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('wo-server-info-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('服务器资源')+"!");
            	return;
            }
            //调用批量删除接口
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('wo-server-info-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('wo-server-info-form-data-form-action', "edit",true);
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('wo-server-info-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('wo-server-info-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('wo-server-info-form-data', data);
		var area=admin.getTempData('wo-server-info-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('服务器资源');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"wo-server-info-form-data-win",
			content: '/business/workorder/server_info/server_info_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
