...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('wo-network-strategy-info-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('wo-network-strategy-info-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'wo-network-strategy-info-form-data-win');
			if(area==null) return;
			admin.putTempData('wo-network-strategy-info-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(sessionType)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=sessionType]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("sessionType",data,checked);
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('wo-network-strategy-info-form-data');
		}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('wo-network-strategy-info-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('wo-network-strategy-info-form-data-win',this); });
