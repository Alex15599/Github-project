...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('wo-network-strategy-apply-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('wo-network-strategy-apply-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'wo-network-strategy-apply-form-data-win');
			if(area==null) return;
			admin.putTempData('wo-network-strategy-apply-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(environmentId)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=environmentId]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("environmentId",data,checked);
		});
		form.on('radio(periodType)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=periodType]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("periodType",data,checked);
		});
		laydate.render({
			elem: '#expirationDate',
			format:"yyyy-MM-dd",
			value:$('#expirationDate').val()?$('#expirationDate').val():new Date(),
			trigger:"click",
		...
		laydate.render({
			elem: '#expectedCompletionDate',
			format:"yyyy-MM-dd",
			value:$('#expectedCompletionDate').val()?$('#expectedCompletionDate').val():new Date(),
			trigger:"click",
			done: function(value, date, endDate){
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("expectedCompletionDate",value, date, endDate);
			}
		});
	    //渲染图片字段
		foxup.render({
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('wo-network-strategy-apply-form-data');
		}
		...
		if (hasData) {
			...
			if(formData["expectedCompletionDate"]) {
				$("#expectedCompletionDate").val(fox.dateFormat(formData["expectedCompletionDate"],"yyyy-MM-dd"));
			}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('wo-network-strategy-apply-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('wo-network-strategy-apply-form-data-win',this); });
