...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('wo-server-info-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('wo-server-info-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'wo-server-info-form-data-win');
			if(area==null) return;
			admin.putTempData('wo-server-info-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(serverType)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=serverType]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("serverType",data,checked);
		});
		//渲染 osVersion 下拉字段
		fox.renderSelectBox({
			el: "osVersion",
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("osVersion",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('wo-server-info-form-data');
		}
		...
		if (hasData) {
			...
			//设置  系统版本 设置下拉框勾选
			fox.setSelectValue4QueryApi("#osVersion",formData.serverOsType);
	...
	function getFormData() {
		...
		//获取 系统版本 下拉框的值
		data["osVersion"]=fox.getSelectedValue("osVersion",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('wo-server-info-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('wo-server-info-form-data-win',this); });
