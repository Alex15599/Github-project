...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('vehicle-info-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('vehicle-info-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'vehicle-info-form-data-win');
			if(area==null) return;
			admin.putTempData('vehicle-info-form-area', area);
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('vehicle-info-form-data');
		}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('vehicle-info-form-data-win',this); });
