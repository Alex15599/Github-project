...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-safety-baseline-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-safety-baseline-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-safety-baseline-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-safety-baseline-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("baseTypeId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-safety-baseline-form-data');
		}
		...
		if (hasData) {
			...
			//设置 检查文件 显示附件
		    if($("#checkFileId").val()) {
				foxup.fill("checkFileId",$("#checkFileId").val());
		    } else {
				adjustPopup();
			}
			...
			//设置  类型 设置下拉框勾选
			fox.setSelectValue4QueryApi("#baseTypeId",formData.baseType);
	...
	function getFormData() {
		...
		//获取 类型 下拉框的值
		data["baseTypeId"]=fox.getSelectedValue("baseTypeId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-safety-baseline-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-safety-baseline-form-data-win',this); });
