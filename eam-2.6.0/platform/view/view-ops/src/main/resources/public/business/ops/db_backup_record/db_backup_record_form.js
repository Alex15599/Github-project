...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-db-backup-record-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-db-backup-record-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-db-backup-record-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-db-backup-record-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("backupResult",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("backupStime",value, date, endDate);
			}
		});
		laydate.render({
			elem: '#backupEtime',
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("backupEtime",value, date, endDate);
			}
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-db-backup-record-form-data');
		}
		...
		if (hasData) {
			...
			//设置 开始时间 显示复选框勾选
			if(formData["backupStime"]) {
				$("#backupStime").val(fox.dateFormat(formData["backupStime"],"yyyy-MM-dd HH:mm:ss"));
			}
			//设置 结束时间 显示复选框勾选
			if(formData["backupEtime"]) {
				$("#backupEtime").val(fox.dateFormat(formData["backupEtime"],"yyyy-MM-dd HH:mm:ss"));
			}


			//设置  备份结果 设置下拉框勾选
			fox.setSelectValue4Enum("#backupResult",formData.backupResult,SELECT_BACKUPRESULT_DATA);
	...
	function getFormData() {
		...
		//获取 备份结果 下拉框的值
		data["backupResult"]=fox.getSelectedValue("backupResult",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-db-backup-record-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-db-backup-record-form-data-win',this); });
