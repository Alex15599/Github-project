...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-ip-address-range-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-ip-address-range-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-ip-address-range-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-ip-address-range-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("regionId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("classId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("prodId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-ip-address-range-form-data');
		}
		...
		if (hasData) {
			...
			//设置  区域 设置下拉框勾选
			fox.setSelectValue4QueryApi("#regionId",formData.regionDict);
			//设置  分类 设置下拉框勾选
			fox.setSelectValue4QueryApi("#classId",formData.classDict);
			//设置  环境 设置下拉框勾选
			fox.setSelectValue4QueryApi("#prodId",formData.prodDict);
	...
	function getFormData() {
		...
		//获取 区域 下拉框的值
		data["regionId"]=fox.getSelectedValue("regionId",false);
		//获取 分类 下拉框的值
		data["classId"]=fox.getSelectedValue("classId",false);
		//获取 环境 下拉框的值
		data["prodId"]=fox.getSelectedValue("prodId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-ip-address-range-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-ip-address-range-form-data-win',this); });
