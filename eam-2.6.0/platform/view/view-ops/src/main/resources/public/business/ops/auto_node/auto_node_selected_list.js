...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-ops/ops-auto-node";
	var dataTable=null;
	var sort=null;
	/**
      * 入口函数，初始化
    ...
    function renderTable() {
		...
		function renderTableInternal() {
			...
			if(OWNER_ID){
				...
				ps.ownerSelectedCode=SELECTED_CODE;
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
			...
			var queryString="?selectedCode="+SELECTED_CODE
			if(OWNER_ID){
				queryString=queryString+"&ownerId="+OWNER_ID;
			}
			var formTop=2
			var index=admin.popupCenter({
				...
				content: '/business/ops/auto_node/auto_node_select_list.html'+queryString,
				finish: function () {

					refreshTableData();
				}
			});
			admin.putTempData('ops-auto-node-select-data-popup-index', index);
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('节点')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('节点')+fox.translate('吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('ops-auto-node-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('ops-auto-node-form-data-form-action', "edit",true);
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('ops-auto-node-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				var pps={};
				pps.selected_code=SELECTED_CODE;
				if(OWNER_ID){
					pps.ownerId=OWNER_ID;
				}
				pps.nodeId=data.id
				top.layer.confirm(fox.translate('确定删除此')+fox.translate('节点')+fox.translate('吗？'), function (i) {
					top.layer.close(i);
					admin.post(moduleURL+"/remove-selected-node", pps, function (data) {
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('ops-auto-node-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('ops-auto-node-form-data', data);
		var area=admin.getTempData('ops-auto-node-form-area');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"ops-auto-node-form-data-win",
			content: '/business/ops/auto_node/auto_node_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
