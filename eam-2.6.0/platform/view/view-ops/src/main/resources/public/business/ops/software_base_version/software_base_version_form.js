...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-software-base-version-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-software-base-version-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-software-base-version-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-software-base-version-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("softwareTypeId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-software-base-version-form-data');
		}
		...
		if (hasData) {
			...
			//设置  类型 设置下拉框勾选
			fox.setSelectValue4QueryApi("#softwareTypeId",formData.softwareBaseType);
			//设置  状态 设置下拉框勾选
	...
	function getFormData() {
		...
		//获取 类型 下拉框的值
		data["softwareTypeId"]=fox.getSelectedValue("softwareTypeId",false);
		//获取 状态 下拉框的值
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-software-base-version-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-software-base-version-form-data-win',this); });
