...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-auto-node-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-auto-node-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-auto-node-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-auto-node-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(authenticationMethod)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=authenticationMethod]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("authenticationMethod",data,checked);
		});
		//渲染 voucherId 下拉字段
		fox.renderSelectBox({
			el: "voucherId",
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("voucherId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-auto-node-form-data');
		}
		...
		if (hasData) {
			...
			//设置  凭证 设置下拉框勾选
			fox.setSelectValue4QueryApi("#voucherId",formData.voucher);
	...
	function getFormData() {
		...
		//获取 凭证 下拉框的值
		data["voucherId"]=fox.getSelectedValue("voucherId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-auto-node-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-auto-node-form-data-win',this); });
