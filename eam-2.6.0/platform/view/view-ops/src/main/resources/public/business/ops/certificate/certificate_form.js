...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-certificate-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-certificate-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-certificate-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-certificate-form-area', area);
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-certificate-form-data');
		}
		...
		if (hasData) {
			...
			//设置  使用位置 设置下拉框勾选
			fox.setSelectValue4QueryApi("#positionId",formData.certificatePosition);
			//设置  类型 设置下拉框勾选
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-certificate-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-certificate-form-data-win',this); });
