...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-monitor-tpl-graph-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-monitor-tpl-graph-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-monitor-tpl-graph-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-monitor-tpl-graph-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("tplCode",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("graphType",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform:function(data) {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-monitor-tpl-graph-form-data');
		}
		...
		if (hasData) {
			...
			//设置  图形模版 设置下拉框勾选
			fox.setSelectValue4QueryApi("#tplCode",formData.tpl);
			//设置  类型 设置下拉框勾选
			fox.setSelectValue4Enum("#graphType",formData.graphType,SELECT_GRAPHTYPE_DATA);
	...
	function getFormData() {
		...
		//获取 图形模版 下拉框的值
		data["tplCode"]=fox.getSelectedValue("tplCode",false);
		//获取 类型 下拉框的值
		data["graphType"]=fox.getSelectedValue("graphType",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-monitor-tpl-graph-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-monitor-tpl-graph-form-data-win',this); });
