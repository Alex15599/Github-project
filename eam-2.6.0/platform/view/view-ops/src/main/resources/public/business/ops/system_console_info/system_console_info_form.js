...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-system-console-info-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-system-console-info-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-system-console-info-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-system-console-info-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("env",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-system-console-info-form-data');
		}
		...
		if (hasData) {
			...
			//设置  所在环境 设置下拉框勾选
			fox.setSelectValue4Dict("#env",formData.env,SELECT_ENV_DATA);
	...
	function getFormData() {
		...
		//获取 所在环境 下拉框的值
		data["env"]=fox.getSelectedValue("env",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-system-console-info-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-system-console-info-form-data-win',this); });
