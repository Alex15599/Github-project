...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-db-instance-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-db-instance-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-db-instance-form-data-win');
			if(area==null) return;
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("databaseId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("backupType",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("backupStatus",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("backupMethod",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
		...
		laydate.render({
			...
			trigger:"click",
				...
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("backupTime",value, date, endDate);
			}
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-db-instance-form-data');
		}
	...
	function getFormData() {
		...
		//获取 数据库 下拉框的值
		data["databaseId"]=fox.getSelectedValue("databaseId",false);
		//获取 日志模式 下拉框的值
		data["logMethod"]=fox.getSelectedValue("logMethod",false);
		//获取 备份状态 下拉框的值
		data["backupType"]=fox.getSelectedValue("backupType",false);
		//获取 备份状态 下拉框的值
		data["backupStatus"]=fox.getSelectedValue("backupStatus",false);
		//获取 备份方式 下拉框的值
		data["backupMethod"]=fox.getSelectedValue("backupMethod",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-db-instance-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-db-instance-form-data-win',this); });
