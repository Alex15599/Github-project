...
function FormPage() {
	...
	this.init=function(layui) {
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-voucher-owner-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-voucher-owner-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-voucher-owner-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("categoryCode",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("label",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-voucher-owner-form-data');
		}
		...
		if (hasData) {
			...
			//设置  类别 设置下拉框勾选
			fox.setSelectValue4QueryApi("#categoryCode",formData.voucherCategory);
			//设置  标签 设置下拉框勾选
			fox.setSelectValue4Dict("#label",formData.label,SELECT_LABEL_DATA);
	...
	function getFormData() {
		...
		//获取 类别 下拉框的值
		data["categoryCode"]=fox.getSelectedValue("categoryCode",false);
		//获取 标签 下拉框的值
		data["label"]=fox.getSelectedValue("label",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-voucher-owner-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-voucher-owner-form-data-win',this); });
