...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-monitor-object-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-monitor-object-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-monitor-object-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-monitor-object-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("objectModelId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("groupCode",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			searchField: "name", //请自行调整用于搜索的字段名称
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-monitor-object-form-data');
		}
		...
		if (hasData) {
			...
			//设置  模型 设置下拉框勾选
			fox.setSelectValue4QueryApi("#objectModelId",formData.objectModel);
			//设置  分组 设置下拉框勾选
			fox.setSelectValue4QueryApi("#groupCode",formData.group);
	...
	function getFormData() {
		...
		//获取 模型 下拉框的值
		data["objectModelId"]=fox.getSelectedValue("objectModelId",false);
		//获取 分组 下拉框的值
		data["groupCode"]=fox.getSelectedValue("groupCode",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-monitor-object-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-monitor-object-form-data-win',this); });
