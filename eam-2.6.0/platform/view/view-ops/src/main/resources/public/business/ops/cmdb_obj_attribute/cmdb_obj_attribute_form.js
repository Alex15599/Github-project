...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-cmdb-obj-attribute-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-cmdb-obj-attribute-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-cmdb-obj-attribute-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-cmdb-obj-attribute-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("attributeColFill",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			searchField: "name", //请自行调整用于搜索的字段名称
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-cmdb-obj-attribute-form-data');
		}
		...
		if (hasData) {
			...
			//设置  字段填充 设置下拉框勾选
			fox.setSelectValue4QueryApi("#attributeColFill",formData.cmdbAttributeC);
	...
	function getFormData() {
		...
		//获取 字段填充 下拉框的值
		data["attributeColFill"]=fox.getSelectedValue("attributeColFill",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-cmdb-obj-attribute-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-cmdb-obj-attribute-form-data-win',this); });
