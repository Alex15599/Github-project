...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-ops/ops-auto-task-log";
	var dataTable=null;
	var sort=null;
	/**
      * 入口函数，初始化
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'recordContent', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('记录结果') , templet: function (d) { return templet('recordContent',d.recordContent,d);}  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		var value = {};
		value.taskId={ inputType:"select_box", value: getSelectedValue("#taskId","value") ,fillBy:["task"]  , label:getSelectedValue("#taskId","nameStr") };
		value.nodeId={ inputType:"select_box", value: getSelectedValue("#nodeId","value") ,fillBy:["node"]  , label:getSelectedValue("#nodeId","nameStr") };
		value.status={ inputType:"select_box", value: getSelectedValue("#status","value"), label:getSelectedValue("#status","nameStr") };
		value.recordContent={ inputType:"button",value: $("#recordContent").val()};
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("nodeId",data.arr,data.change,data.isAdd);
				},1);
			},
			paging: true,
			pageRemote: true,
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('ops-auto-task-log-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('ops-auto-task-log-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('明细日志')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('明细日志')+fox.translate('吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('ops-auto-task-log-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('ops-auto-task-log-form-data-form-action', "edit",true);
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('ops-auto-task-log-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此')+fox.translate('明细日志')+fox.translate('吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('ops-auto-task-log-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('ops-auto-task-log-form-data', data);
		var area=admin.getTempData('ops-auto-task-log-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('明细日志');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"ops-auto-task-log-form-data-win",
			content: '/business/ops/auto_task_log/auto_task_log_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
