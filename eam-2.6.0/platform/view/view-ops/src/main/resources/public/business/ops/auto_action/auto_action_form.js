...
function FormPage() {
	...
	const moduleURL="/service-ops/ops-auto-action";
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-auto-action-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-auto-action-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-auto-action-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-auto-action-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(fileStatus)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=fileStatus]:checked').each(function() {
				checked.push($(this).val());
			});
		...
		form.on('radio(executeTool)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=executeTool]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("executeTool",data,checked);
		});
		//渲染 autoActionScriptIds 下拉字段
		fox.renderSelectBox({
			el: "autoActionScriptIds",
			radio: false,
			filterable: true,
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("autoActionScriptIds",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
		...
		//渲染 autoActionFileIds 下拉字段
		fox.renderSelectBox({
			el: "autoActionFileIds",
			radio: false,
			filterable: true,
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("autoActionFileIds",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			searchField: "name", //请自行调整用于搜索的字段名称
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-auto-action-form-data');
		}
		...
		if (hasData) {
			...
			//设置  脚本 设置下拉框勾选
			fox.setSelectValue4QueryApi("#autoActionScriptIds",formData.autoActionScriptList);
			//设置  介质 设置下拉框勾选
			fox.setSelectValue4QueryApi("#autoActionFileIds",formData.autoActionFileList);
	...
	function getFormData() {
		...
		//获取 脚本 下拉框的值
		data["autoActionScriptIds"]=fox.getSelectedValue("autoActionScriptIds",true);
		//获取 介质 下拉框的值
		data["autoActionFileIds"]=fox.getSelectedValue("autoActionFileIds",true);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-auto-action-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-auto-action-form-data-win',this); });
