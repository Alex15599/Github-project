...
function FormPage() {
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-db-backup-info-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-db-backup-info-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(backupType)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=backupType]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("backupType",data,checked);
		});
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-db-backup-info-form-data');
		}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-db-backup-info-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-db-backup-info-form-data-win',this); });
