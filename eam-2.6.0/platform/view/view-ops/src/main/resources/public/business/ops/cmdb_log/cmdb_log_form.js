...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('ops-cmdb-log-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('ops-cmdb-log-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'ops-cmdb-log-form-data-win');
			if(area==null) return;
			admin.putTempData('ops-cmdb-log-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("modelId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("objId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			searchField: "name", //请自行调整用于搜索的字段名称
		...
		form.on('radio(logLevel)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=logLevel]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("logLevel",data,checked);
		});
		laydate.render({
			elem: '#rcdTime',
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('ops-cmdb-log-form-data');
		}
		...
		if (hasData) {
			...
			//设置  模型 设置下拉框勾选
			fox.setSelectValue4QueryApi("#modelId",formData.cmdbModel);
			//设置  对象 设置下拉框勾选
			fox.setSelectValue4QueryApi("#objId",formData.cmdbModelV);
	...
	function getFormData() {
		...
		//获取 模型 下拉框的值
		data["modelId"]=fox.getSelectedValue("modelId",false);
		//获取 对象 下拉框的值
		data["objId"]=fox.getSelectedValue("objId",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('ops-cmdb-log-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('ops-cmdb-log-form-data-win',this); });
