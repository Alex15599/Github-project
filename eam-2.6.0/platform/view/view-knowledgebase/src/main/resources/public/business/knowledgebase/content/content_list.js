...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'categoryId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('分类'), templet: function (d) { return templet('categoryId' ,fox.joinLabel(d.category,"hierarchyName",',','','categoryId'),d);}}
					...
					,{ field: 'contentType', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('内容类型'), templet:function (d){ return templet('contentType',fox.getEnumText(RADIO_CONTENTTYPE_DATA,d.contentType,'','contentType'),d);}}
					...
					,{ field: 'display', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('是否显示'), templet:function (d){ return templet('display',fox.getEnumText(RADIO_DISPLAY_DATA,d.display,'','display'),d);}}
					,{ field: 'gradeId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('等级') , templet: function (d) { return templet('gradeId',d.gradeId,d);}  }
					,{ field: 'createTime', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('创建时间') ,templet: function (d) { return templet('createTime',fox.dateFormat(d.createTime,"yyyy-MM-dd HH:mm:ss"),d); }  }
					,{ field: 'resEditorId', align:"",fixed:false,  hide:false, sort: false  , title: fox.translate('编辑人') , templet: function (d) { return templet('resEditorId',fox.getProperty(d,["editor","name"],0,'','resEditorId'),d);} }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.profile={ inputType:"button",value: $("#profile").val() ,fuzzy: true,splitValue:false,valuePrefix:"",valueSuffix:"" };
		value.contentType={ inputType:"radio_box", value: getSelectedValue("#contentType","value"), label:getSelectedValue("#contentType","nameStr") };
		value.display={ inputType:"radio_box", value: getSelectedValue("#display","value"), label:getSelectedValue("#display","nameStr") };
		value.gradeId={ inputType:"button",value: $("#gradeId").val()};
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function initSearchFields() {
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("categoryId",{data:data[i],name:data[i].hierarchyName,value:data[i].id},data[i],data,i));
					} else {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("contentType",data.arr,data.change,data.isAdd);
				},1);
			},
			...
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("contentType",{data:data[i],name:data[i].text,value:data[i].code},data[i],data,i));
					} else {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("display",data.arr,data.change,data.isAdd);
				},1);
			},
			//toolbar: {show:true,showIcon:true,list:["CLEAR","REVERSE"]},
			transform:function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(window.pageExt.list.selectBoxDataTransform) {
						opts.push(window.pageExt.list.selectBoxDataTransform("display",{data:data[i],name:data[i].text,value:data[i].code},data[i],data,i));
					} else {
						opts.push({data:data[i],name:data[i].text,value:data[i].code});
					}
				}
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('kn-content-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'知识库内容'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'知识库内容'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				top.layer.confirm(fox.translate('确定删除此'+'知识库内容'+'吗？'), function (i) {
					top.layer.close(i);
				...
				window.pageExt.list.reviewKnFunction(data,this);
			}
	...
	function showEditForm(data) {
		...
		admin.popupCenter({
			...
			content: '/business/knowledgebase/content/content_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
