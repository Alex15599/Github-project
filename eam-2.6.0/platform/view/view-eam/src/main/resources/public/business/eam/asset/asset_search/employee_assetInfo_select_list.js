...
function ListPage() {
    ...
    function renderTable() {
        ...
        function renderTableInternal() {
            ...
            COL_DATA.push({fixed: 'left',type:'checkbox'});
        ...
        table.on('toolbar(data-table)', function(obj){
            ...
            switch(obj.event){
                case 'create':
                    ...
                    var ps={};
                    if(ids.length==0){
                        top.layer.msg("请选择资产数据", {time: 2000});
                        return;
                    }
                    ps.assetBusinessType="employee_data";
                    ps.assetSelectedCode=SELECTED_CODE;
                    ps.ids=ids;
                    ps.ownerCode="asset";
                    ps.assetOwnerId=OWNER_ID;
                    admin.post("/service-eam/eam-asset-selected-data/save-by-ids", ps, function (r) {
                        if (r.success) {
                            top.layer.msg(r.message, {time: 2000});
                            admin.finishPopupCenterById('eam-asset-employee-select-form-data-win');
                        } else {
                            top.layer.msg(r.message, {time: 2000});
                        }
                    }, {delayLoading: 1000, elms: null});
                    break;
                case 'batch-del':
