...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-eam/eam-c-cust-repiar-item";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'repairId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('报修') , templet: function (d) { return templet('repairId',d.repairId,d);}  }
					,{ field: 'processUserId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('处理人') , templet: function (d) { return templet('processUserId',fox.getProperty(d,["processUser","name"],0,'','processUserId'),d);} }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		var value = {};
		value.repairId={ inputType:"button",value: $("#repairId").val()};
		value.processUserId={ inputType:"button",value: $("#processUserId").val(),fillBy:["processUser","name"] };
		value.ct={ inputType:"button",value: $("#ct").val()};
	...
	function bindButtonEvent() {
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'内容'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'内容'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-c-cust-repiar-item-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-c-cust-repiar-item-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-c-cust-repiar-item-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此'+'内容'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-c-cust-repiar-item-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-c-cust-repiar-item-form-data', data);
		var area=admin.getTempData('eam-c-cust-repiar-item-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('内容');
		...
		admin.popupCenter({
			...
			content: '/business/eam/c_cust_repiar_item/c_cust_repiar_item_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
