...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'code', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('项目编号') , templet: function (d) { return templet('code',d.code,d);}  }
					...
					,{ field: 'maintainType', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('保养类型'), templet: function (d) { return templet('maintainType' ,fox.joinLabel(d.maintainTypeDict,"label"),d);}}
					,{ field: 'baseCose', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('标准工时(小时)') , templet: function (d) { return templet('baseCose',d.baseCose,d);}  }
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					...
					ps.ids=ids;
					//调用批量删除接口
					top.layer.confirm(fox.translate('确定选择已选中的')+fox.translate('保养项目')+fox.translate('吗？'), function (i) {
						admin.post(moduleURL+"/selected", ps, function (data) {
							if (data.success) {
								admin.finishPopupCenterById('eam-maintain-project-select-list-data-win');
								fox.showMessage(data);
							} else {
								fox.showMessage(data);
							}
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-maintain-project-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
			...
			var ids=getCheckedList("id");
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('保养项目')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('保养项目')+fox.translate('吗？'), function (i) {
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-maintain-project-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-maintain-project-form-data-form-action', "edit",true);
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-maintain-project-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此')+fox.translate('保养项目')+fox.translate('吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-maintain-project-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-maintain-project-form-data', data);
		var area=admin.getTempData('eam-maintain-project-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('保养项目');
