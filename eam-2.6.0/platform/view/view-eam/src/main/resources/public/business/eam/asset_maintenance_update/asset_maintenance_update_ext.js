    ...
    var list={
        ...
        afterQuery : function (data) {
            for (var i = 0; i < data.length; i++) {
                //如果审批中或审批通过的不允许编辑
                if(data[i].handleStatus=="finish") {
                //    fox.disableButton($('.update-detail-button').filter("[data-id='" + data[i].id + "']"), true);
                    fox.disableButton($('.ops-delete-button').filter("[data-id='" + data[i].id + "']"), true);
                    fox.disableButton($('.ops-edit-button').filter("[data-id='" + data[i].id + "']"), true);

                }else if(data[i].handleStatus=="cancel"){
                    fox.disableButton($('.update-detail-button').filter("[data-id='" + data[i].id + "']"), true);
                    fox.disableButton($('.ops-delete-button').filter("[data-id='" + data[i].id + "']"), true);
                    fox.disableButton($('.ops-edit-button').filter("[data-id='" + data[i].id + "']"), true);
                }else if(data[i].handleStatus=="draft"){
                    // fox.disableButton($('.update-detail-button').filter("[data-id='" + data[i].id + "']"), true);
                    // fox.disableButton($('.ops-delete-button').filter("[data-id='" + data[i].id + "']"), true);
                    // fox.disableButton($('. ops-edit-button').filter("[data-id='" + data[i].id + "']"), true);
                }
        ...
        updateDetail:function (data){
            ...
            var index=admin.popupCenter({
                ...
                area: ["95%","95%"],
                type: 2,
                id:"eam-asset-maintenance-record-list-data-win",
                content: '/business/eam/asset_maintenance_record/asset_maintenance_record_list.html?maintenanceUpdateId='+data.id,
                finish: function () {
                    console.log('11');
                }
            });
            admin.putTempData('eam-asset-maintenance-record-data-popup-index', index);
        },
        /**
         * 末尾执行
         */
