...
function ListPage() {
	...
	function initSearchFields() {
		...
		laydate.render({
			elem: '#scrapDate-begin',
			trigger:"click",
			done: function(value, date, endDate) {
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("scrapDate",value, date, endDate);
				},1);
			}
		});
		laydate.render({
	...
	function showEditForm(data) {
		...
		admin.popupCenter({
			...
			content: '/business/eam/asset_scrap/asset_scrap_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
