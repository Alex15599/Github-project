...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-eam/eam-asset-employee-loss";
	var dataTable=null;
	var sort=null;
	/**
      * 入口函数，初始化
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'content', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('报失原因') , templet: function (d) { return templet('content',d.content,d);}  }
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-asset-employee-loss-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('资产报失')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('资产报失')+fox.translate('吗？'), function (i) {
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-asset-employee-loss-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				bpm.getProcessInstanceByBill("eam_asset_employee_loss",{ id : data.id },function(p) {
					if(p) {
						bpm.openProcessView(p.id,null,false,{"formDefinitionCode":"eam_asset_employee_loss"},refreshTableData,refreshRowData,"bill");
					} else {
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-asset-employee-loss-form-data-form-action', "view",true);
