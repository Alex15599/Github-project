...
function FormPage() {
	...
	this.init=function(layui) {
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-asset-employee-handover-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-asset-employee-handover-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-asset-employee-handover-form-area', area);
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-asset-employee-handover-form-data');
		}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-asset-employee-handover-form-data-win');
				}
    ...
    function bindButtonEvent() {
		...
		// 请选择组织节点对话框
		$("#receiveOrgId-button").click(function(){
			var receiveOrgIdDialogOptions={
				field:"receiveOrgId",
				formData:getFormData(),
				inputEl:$("#receiveOrgId"),
				buttonEl:$(this),
				single:true,
				//限制浏览的范围，指定根节点 id 或 code ，优先匹配ID
				root: "",
			...
			fox.chooseOrgNode(receiveOrgIdDialogOptions);
		});
		// 请选择人员对话框
		$("#receiveUserId-button").click(function(){
				var receiveUserIdDialogOptions={
				field:"receiveUserId",
				formData:getFormData(),
				inputEl:$("#receiveUserId"),
				buttonEl:$(this),
				single:true,
				//限制浏览的范围，指定根节点 id 或 code ，优先匹配ID
				root: "",
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-asset-employee-handover-form-data-win',this); });
