...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-asset-maintenance-update-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-asset-maintenance-update-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-asset-maintenance-update-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-asset-maintenance-update-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(handleStatus)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=handleStatus]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("handleStatus",data,checked);
		});
	    //渲染图片字段
		foxup.render({
			el:"attachId",
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-asset-maintenance-update-form-data');
		}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-asset-maintenance-update-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-asset-maintenance-update-form-data-win',this); });
