...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'name', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('维保名称') , templet: function (d) { return templet('name',d.name,d);}  }
					...
					,{ field: 'useOrgId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('负责部门') , templet: function (d) { return templet('useOrgId',fox.getProperty(d,["useOrganization","fullName"]),d);} }
					,{ field: 'managerId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('维保负责人') , templet: function (d) { return templet('managerId',fox.getProperty(d,["manager","name"]),d);} }
					,{ field: 'contacts', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('维保联系人') , templet: function (d) { return templet('contacts',d.contacts,d);}  }
					,{ field: 'contactInfo', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('联系信息') , templet: function (d) { return templet('contactInfo',d.contactInfo,d);}  }
					,{ field: 'maintenanceCost', align:"right",fixed:false,  hide:false, sort: true  , title: fox.translate('维保费用') , templet: function (d) { return templet('maintenanceCost',d.maintenanceCost,d);}  }
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-asset-software-maintenance-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('软件维保')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('软件维保')+fox.translate('吗？'), function (i) {
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-asset-software-maintenance-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-asset-software-maintenance-form-data-form-action', "edit",true);
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-asset-software-maintenance-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此')+fox.translate('软件维保')+fox.translate('吗？'), function (i) {
					top.layer.close(i);
