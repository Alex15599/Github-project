...
function ListPage() {
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("roleCode",data.arr,data.change,data.isAdd);
				},1);
			},
			paging: true,
			pageRemote: true,
	...
	function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-asset-data-permissions-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
	...
	function showEditForm(data) {
		...
		var title = fox.translate('资产数据权限');
