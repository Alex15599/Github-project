...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'orgId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('申请部门') , templet: function (d) { return templet('orgId',fox.getProperty(d,["organization","fullName"]),d);} }
					,{ field: 'content', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('原因') , templet: function (d) { return templet('content',d.content,d);}  }
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					...
					bpm.openProcessView(null,null,false,{"formDefinitionCode":"eam_asset_employee_apply"},refreshTableData,refreshRowData,"bill",defaultValue);
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-asset-employee-apply-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('领用申请')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('领用申请')+fox.translate('吗？'), function (i) {
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-asset-employee-apply-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				bpm.getProcessInstanceByBill("eam_asset_employee_apply",{ id : data.id },function(p) {
					if(p) {
						bpm.openProcessView(p.id,null,false,{"formDefinitionCode":"eam_asset_employee_apply"},refreshTableData,refreshRowData,"bill");
					} else {
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-asset-employee-apply-form-data-form-action', "view",true);
