...
layui.define(['form', 'table', 'util', 'settings', 'admin', 'upload','foxnic','xmSelect','laydate','foxnicUpload','dropdown'],function () {
    ...
    var formAction=admin.getTempData('eam-asset-stock-deliver-form-data-form-action');
    ...
    var list={
        ...
        afterQuery : function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                //如果审批中或审批通过的不允许编辑
                fox.disableButton($('.download-bill-button').filter("[data-id='" + data[i].id + "']"), true);
