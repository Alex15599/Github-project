...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-eam/eam-c-cust-inspect-plan";
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-c-cust-inspect-plan-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-c-cust-inspect-plan-form-data-form-action', "create",true);
            showEditForm(data);
        };
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-c-cust-inspect-plan-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-c-cust-inspect-plan-form-data-form-action', "edit",true);
				...
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-c-cust-inspect-plan-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
			...
			else if (layEvent === 'action-execute') { // 执行
				window.pageExt.list.actionExecute(data,this);
			}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-c-cust-inspect-plan-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-c-cust-inspect-plan-form-data', data);
		var area=admin.getTempData('eam-c-cust-inspect-plan-form-area');
		...
		admin.popupCenter({
			...
			area: ["60%",height+"px"],
			type: 2,
			id:"eam-c-cust-inspect-plan-form-data-win",
			content: '/business/eam/c_cust_inspect_plan/c_cust_inspect_plan_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
