...
function ListPage() {
	...
	function bindSearchEvent() {
		...
		// 请选择组织节点对话框
		$("#applyOrgId-button").click(function(){
			var applyOrgIdDialogOptions={
				field:"applyOrgId",
				inputEl:$("#applyOrgId"),
				buttonEl:$(this),
				single:false,
			...
			fox.chooseOrgNode(applyOrgIdDialogOptions);
		});
	}
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-purchase-apply-form-data', {});
					var defaultValue={};
					if(window.pageExt.list.getBpmViewConfig) {
						defaultValue=window.pageExt.list.getBpmViewConfig();
					}
					bpm.openProcessView(null,null,false,{"formDefinitionCode":"eam_asset_purchase_apply"},refreshTableData,refreshRowData,"bill",defaultValue);
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-purchase-apply-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('采购申请')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('采购申请')+fox.translate('吗？'), function (i) {
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-purchase-apply-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				bpm.getProcessInstanceByBill("eam_asset_purchase_apply",{ id : data.id },function(p) {
					if(p) {
						bpm.openProcessView(p.id,null,false,{"formDefinitionCode":"eam_asset_purchase_apply"},refreshTableData,refreshRowData,"bill");
					} else {
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-purchase-apply-form-data-form-action', "view",true);
				...
				window.pageExt.list.check(data,this);
			}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-purchase-apply-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-purchase-apply-form-data', data);
		var area=admin.getTempData('eam-purchase-apply-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('采购申请');
