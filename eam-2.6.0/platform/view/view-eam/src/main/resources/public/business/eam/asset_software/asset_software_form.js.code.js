...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-asset-software-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-asset-software-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-asset-software-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-asset-software-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("copyrightType",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("licenseMode",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
		...
		form.on('radio(authorizedNumberUnlimit)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=authorizedNumberUnlimit]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("authorizedNumberUnlimit",data,checked);
		});
		laydate.render({
			elem: '#authorizationExpirationDate',
			format:"yyyy-MM-dd",
			trigger:"click",
			done: function(value, date, endDate){
				window.pageExt.form.onDatePickerChanged && window.pageExt.form.onDatePickerChanged("authorizationExpirationDate",value, date, endDate);
			}
		});
		form.on('radio(authorizationExpirationUnlimit)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=authorizationExpirationUnlimit]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("authorizationExpirationUnlimit",data,checked);
		});
		form.on('radio(needMaintenance)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=needMaintenance]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("needMaintenance",data,checked);
		});
		...
		laydate.render({
			elem: '#registerDate',
			format:"yyyy-MM-dd",
			value:$('#registerDate').val()?$('#registerDate').val():new Date(),
			trigger:"click",
			done: function(value, date, endDate){
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-asset-software-form-data');
		}
		...
		if (hasData) {
			...
			//设置 授权到期时间 显示复选框勾选
			if(formData["authorizationExpirationDate"]) {
				$("#authorizationExpirationDate").val(fox.dateFormat(formData["authorizationExpirationDate"],"yyyy-MM-dd"));
			}
			//设置 维保开始时间 显示复选框勾选
			...
			//设置  版权类型 设置下拉框勾选
			fox.setSelectValue4QueryApi("#copyrightType",formData.copyrightTypeDict);
			//设置  许可模式 设置下拉框勾选
			fox.setSelectValue4QueryApi("#licenseMode",formData.licenseModeDict);
			//设置  供应商 设置下拉框勾选
	...
	function getFormData() {
		...
		//获取 版权类型 下拉框的值
		data["copyrightType"]=fox.getSelectedValue("copyrightType",false);
		//获取 许可模式 下拉框的值
		data["licenseMode"]=fox.getSelectedValue("licenseMode",false);
		//获取 供应商 下拉框的值
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-asset-software-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-asset-software-form-data-win',this); });
