...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-asset-status-rule-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-asset-status-rule-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-asset-status-rule-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-asset-status-rule-form-area', area);
	...
	function renderFormFields() {
		...
		form.on('radio(operCondition)', function(data){
			var checked=[];
			$('input[type=radio][lay-filter=operCondition]:checked').each(function() {
				checked.push($(this).val());
			});
			window.pageExt.form.onRadioBoxChanged && window.pageExt.form.onRadioBoxChanged("operCondition",data,checked);
		});
	}
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-asset-status-rule-form-data');
		}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-asset-status-rule-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-asset-status-rule-form-data-win',this); });
