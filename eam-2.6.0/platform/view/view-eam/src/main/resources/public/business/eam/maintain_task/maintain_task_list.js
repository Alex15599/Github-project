...
function ListPage() {
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-maintain-task-form-data-form-action', "create",true);
            showEditForm(data);
        };
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-maintain-task-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-maintain-task-form-data', data);
		var area=admin.getTempData('eam-maintain-task-form-area');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"eam-maintain-task-form-data-win",
			content: '/business/eam/maintain_task/maintain_task_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
