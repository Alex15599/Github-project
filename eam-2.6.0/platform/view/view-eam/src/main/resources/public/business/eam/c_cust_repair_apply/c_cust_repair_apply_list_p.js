...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'planFinishDate', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('报修日期') ,templet: function (d) { return templet('planFinishDate',fox.dateFormat(d.planFinishDate,"yyyy-MM-dd HH:mm:ss"),d); }  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.startDate={ inputType:"date_input", begin: $("#startDate-begin").val(), end: $("#startDate-end").val() ,matchType:"auto" };
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function initSearchFields() {
		...
		laydate.render({
			elem: '#startDate-begin',
			trigger:"click",
			done: function(value, date, endDate) {
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("startDate",value, date, endDate);
				},1);
			}
		});
		laydate.render({
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-c-cust-repair-apply-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-c-cust-repair-apply-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'报修申请'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'报修申请'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-c-cust-repair-apply-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(getByIdURL, { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-c-cust-repair-apply-form-data-form-action', "edit",true);
				...
				top.layer.confirm(fox.translate('确定删除此'+'报修申请'+'吗？'), function (i) {
					top.layer.close(i);
			...
			else if (layEvent === 'view-full') { // 查看
				window.pageExt.list.viewFull(data,this);
			}
			else if (layEvent === 'receive-action') { // 接单
				window.pageExt.list.receiveAction(data,this);
			}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-c-cust-repair-apply-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-c-cust-repair-apply-form-data', data);
		var area=admin.getTempData('eam-c-cust-repair-apply-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('报修申请');
		...
		admin.popupCenter({
			...
			content: '/business/eam/c_cust_repair_apply/c_cust_repair_apply_form_public.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
