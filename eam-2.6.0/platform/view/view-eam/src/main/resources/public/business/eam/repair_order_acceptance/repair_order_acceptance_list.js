...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-eam/eam-repair-order-acceptance";
	var dataTable=null;
	var sort=null;
	/**
      * 入口函数，初始化
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.resultType={ inputType:"select_box", value: getSelectedValue("#resultType","value") ,fillBy:["resultTypeDict"]  , label:getSelectedValue("#resultType","nameStr") };
		value.categoryTplId={ inputType:"select_box", value: getSelectedValue("#categoryTplId","value") ,fillBy:["categoryTpl"]  , label:getSelectedValue("#categoryTplId","nameStr") };
		value.finishTime={ inputType:"date_input", begin: $("#finishTime-begin").val(), end: $("#finishTime-end").val() ,matchType:"auto" };
	...
	function initSearchFields() {
		...
		laydate.render({
			elem: '#finishTime-begin',
			trigger:"click",
			done: function(value, date, endDate) {
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("finishTime",value, date, endDate);
				},1);
			}
		});
		laydate.render({
	...
	function bindButtonEvent() {
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('维修验收')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('维修验收')+fox.translate('吗？'), function (i) {
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-repair-order-acceptance-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-repair-order-acceptance-form-data-form-action', "edit",true);
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-repair-order-acceptance-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
				...
				top.layer.confirm(fox.translate('确定删除此')+fox.translate('维修验收')+fox.translate('吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-repair-order-acceptance-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-repair-order-acceptance-form-data', data);
		var area=admin.getTempData('eam-repair-order-acceptance-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('维修验收');
