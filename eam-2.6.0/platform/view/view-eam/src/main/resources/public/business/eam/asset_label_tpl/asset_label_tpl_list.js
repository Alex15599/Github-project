...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'imageType', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('图像类型txm') , templet: function (d) { return templet('imageType',d.imageType,d);}  }
					...
					,{ field: 'keyBold', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('表格内Key是否加粗') , templet: function (d) { return templet('keyBold',d.keyBold,d);}  }
					,{ field: 'valueBlod', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('表格内Value是否加粗') , templet: function (d) { return templet('valueBlod',d.valueBlod,d);}  }
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-asset-label-tpl-form-data-form-action', "create",true);
            showEditForm(data);
        };
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-asset-label-tpl-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-asset-label-tpl-form-data-form-action', "edit",true);
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-asset-label-tpl-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-asset-label-tpl-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-asset-label-tpl-form-data', data);
		var area=admin.getTempData('eam-asset-label-tpl-form-area');
