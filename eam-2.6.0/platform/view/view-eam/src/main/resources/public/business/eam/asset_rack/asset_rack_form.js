 ...
 * @since 2022-08-27 23:54:05
 */

function FormPage() {

	var settings,admin,form,table,layer,util,fox,upload,xmSelect,foxup,dropdown;
	const moduleURL="/service-eam/eam-asset-rack";
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-asset-rack-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-asset-rack-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-asset-rack-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-asset-rack-form-area', area);
		...
		fox.renderSelectBox({
			...
			transform: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					if(!data[i]) continue;
					opts.push({data:data[i],name:data[i].rackName,value:data[i].id,selected:(defaultValues.indexOf(data[i].id)!=-1 || defaultIndexs.indexOf(""+i)!=-1)});
				}
				return opts;
			}
		});
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-asset-rack-form-data');
		}
		...
		if (hasData) {
			...
			//设置  机柜 设置下拉框勾选
			fox.setSelectValue4QueryApi("#rackId",formData.rackInfo);
	...
	function bindButtonEvent() {
		...
		//关闭窗口
		$("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-asset-rack-form-data-win',this); });
