...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-purchase-apply-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-purchase-apply-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-purchase-apply-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-purchase-apply-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("assetCheck",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform:function(data) {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-purchase-apply-form-data');
		}
		...
		if (hasData) {
			...
			if(formData["expectedArrivalDate"]) {
				$("#expectedArrivalDate").val(fox.dateFormat(formData["expectedArrivalDate"],"yyyy-MM-dd"));
			}
			//设置 申请日期 显示复选框勾选
			if(formData["applyDate"]) {
				$("#applyDate").val(fox.dateFormat(formData["applyDate"],"yyyy-MM-dd"));
			}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-purchase-apply-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-purchase-apply-form-data-win',this); });
