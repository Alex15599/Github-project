...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'receiveOrgId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('交接部门') , templet: function (d) { return templet('receiveOrgId',fox.getProperty(d,["receiveOrganization","fullName"]),d);} }
					,{ field: 'receiveUserId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('交接人') , templet: function (d) { return templet('receiveUserId',fox.getProperty(d,["receiverUser","name"]),d);} }
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-asset-employee-handover-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('资产交接')+"!");
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的')+fox.translate('资产交接')+fox.translate('吗？'), function (i) {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-asset-employee-handover-form-data-form-action', "view",true);
