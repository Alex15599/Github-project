...
function FormPage() {
	...
	function renderFormFields() {
		...
		//渲染 residualValueSelect 下拉字段
		fox.renderSelectBox({
			el: "residualValueSelect",
			radio: true,
			filterable: false,
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("residualValueSelect",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform:function(data) {
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-asset-depreciation-form-data-win',this); });
