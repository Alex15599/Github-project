...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-eam/eam-action-crontab";
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'lastExecutionTime', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('上次执行时间') ,templet: function (d) { return templet('lastExecutionTime',fox.dateFormat(d.lastExecutionTime,"yyyy-MM-dd HH:mm:ss"),d); }  }
					,{ field: 'nextExecutionTime', align:"right", fixed:false, hide:false, sort: true   ,title: fox.translate('下次执行时间') ,templet: function (d) { return templet('nextExecutionTime',fox.dateFormat(d.nextExecutionTime,"yyyy-MM-dd HH:mm:ss"),d); }  }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.crontab={ inputType:"button",value: $("#crontab").val()};
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-action-crontab-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'执行动作'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'执行动作'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-action-crontab-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				...
				top.layer.confirm(fox.translate('确定删除此'+'执行动作'+'吗？'), function (i) {
					top.layer.close(i);
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-action-crontab-form-data-form-action');
		var queryString="";
		...
		admin.popupCenter({
			...
			content: '/business/eam/action_crontab/action_crontab_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
