...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-eam/eam-maintain-task-project";
	var dataTable=null;
	var sort=null;
	/**
      * 入口函数，初始化
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.projectMaintainType={ inputType:"select_box", value: getSelectedValue("#projectMaintainType","value") ,fillBy:["maintainTypeDict"]  , label:getSelectedValue("#projectMaintainType","nameStr") };
		var ps={searchField:"$composite"};
		if(window.pageExt.list.beforeQuery){
	...
	function initSearchFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.list.onSelectBoxChanged && window.pageExt.list.onSelectBoxChanged("projectMaintainType",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
	...
	function bindButtonEvent() {
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-maintain-task-project-form-data-form-action', "create",true);
            showEditForm(data);
        };
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-maintain-task-project-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-maintain-task-project-form-data-form-action', "edit",true);
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-maintain-task-project-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-maintain-task-project-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-maintain-task-project-form-data', data);
		var area=admin.getTempData('eam-maintain-task-project-form-area');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"eam-maintain-task-project-form-data-win",
			content: '/business/eam/maintain_task_project/maintain_task_project_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
