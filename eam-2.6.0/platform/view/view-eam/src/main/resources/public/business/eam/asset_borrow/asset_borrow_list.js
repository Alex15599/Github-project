...
function ListPage() {
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'borrowStatus', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('借用状态'), templet:function (d){ return templet('borrowStatus',fox.getEnumText(SELECT_BORROWSTATUS_DATA,d.borrowStatus,'','borrowStatus'),d);}}
					...
					,{ field: 'borrowerId', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('借用人') , templet: function (d) { return templet('borrowerId',fox.getProperty(d,["borrower","name"],0,'','borrowerId'),d);} }
	...
	function refreshTableData(sortField,sortType,reset) {
		...
		value.borrowTime={ inputType:"date_input", begin: $("#borrowTime-begin").val(), end: $("#borrowTime-end").val() ,matchType:"auto" };
	...
	function initSearchFields() {
		...
		laydate.render({
			...
			trigger:"click",
				...
				setTimeout(function () {
					window.pageExt.list.onDatePickerChanged && window.pageExt.list.onDatePickerChanged("borrowTime",value, date, endDate);
				},1);
			}
		});
		laydate.render({
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-asset-borrow-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
					batchDelete(selected);
					break;
				case 'tool-asset-return':
					window.pageExt.list.assetReturn && window.pageExt.list.assetReturn(selected,obj);
					break;
				case 'refresh-data':
					refreshTableData();
					break;
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的'+'资产借用'+"!"));
            	return;
            }
            //调用批量删除接口
			top.layer.confirm(fox.translate('确定删除已选中的'+'资产借用'+'吗？'), function (i) {
                top.layer.close(i);
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			if (layEvent === 'edit') { // 修改
				...
				top.layer.confirm(fox.translate('确定删除此'+'资产借用'+'吗？'), function (i) {
					top.layer.close(i);
			...
			else if (layEvent === 'download-return-bill') { // 归还单据
				window.pageExt.list.downloadReturnBill(data,this);
			}
	...
	function showEditForm(data) {
		...
		admin.popupCenter({
			...
			content: '/business/eam/asset_borrow/asset_borrow_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
