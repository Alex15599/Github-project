...
function ListPage() {
	...
	//模块基础路径
	const moduleURL="/service-datacenter/eam-asset-rack-info";
	var dataTable=null;
	var sort=null;
	/**
      * 入口函数，初始化
		...
		function renderTableInternal() {
			...
			var tableConfig={
				...
				cols: [[
					...
					,{ field: 'rackType', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('类型'), templet: function (d) { return templet('rackType' ,fox.joinLabel(d.typeDict,"label",',','','rackType'),d);}}
					...
					,{ field: 'status', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('状态'), templet: function (d) { return templet('status' ,fox.joinLabel(d.statusDict,"label",',','','status'),d);}}
					,{ field: 'environment', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('环境'), templet: function (d) { return templet('environment' ,fox.joinLabel(d.environmentDict,"label",',','','environment'),d);}}
					,{ field: 'rackUsedType', align:"left",fixed:false,  hide:false, sort: true  , title: fox.translate('使用分类'), templet: function (d) { return templet('rackUsedType' ,fox.joinLabel(d.usedTypeDict,"label",',','','rackUsedType'),d);}}
	...
	function bindButtonEvent() {
		...
		table.on('toolbar(data-table)', function(obj){
			...
			switch(obj.event){
				case 'create':
					admin.putTempData('eam-asset-rack-info-form-data', {});
					openCreateFrom();
					break;
				case 'batch-del':
        ...
        function openCreateFrom() {
        	//设置新增是初始化数据
        	var data={};
			admin.putTempData('eam-asset-rack-info-form-data-form-action', "create",true);
            showEditForm(data);
        };
        ...
        function batchDelete(selected) {
            ...
            if(ids.length==0) {
				top.layer.msg(fox.translate('请选择需要删除的')+fox.translate('机柜')+"!");
            	return;
            }
            //调用批量删除接口
    ...
    function bindRowOperationEvent() {
		...
		table.on('tool(data-table)', function (obj) {
			...
			admin.putTempData('eam-asset-rack-info-form-data-form-action', "",true);
			if (layEvent === 'edit') { // 修改
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-asset-rack-info-form-data-form-action', "edit",true);
				...
				admin.post(moduleURL+"/get-by-id", { id : data.id }, function (data) {
					if(data.success) {
						admin.putTempData('eam-asset-rack-info-form-data-form-action', "view",true);
						showEditForm(data.data);
					} else {
						fox.showMessage(data);
					}
	...
	function showEditForm(data) {
		...
		var action=admin.getTempData('eam-asset-rack-info-form-data-form-action');
		var queryString="";
		if(data && data.id) queryString='id=' + data.id;
		if(window.pageExt.list.makeFormQueryString) {
			queryString=window.pageExt.list.makeFormQueryString(data,queryString,action);
		}
		admin.putTempData('eam-asset-rack-info-form-data', data);
		var area=admin.getTempData('eam-asset-rack-info-form-area');
		var height= (area && area.height) ? area.height : ($(window).height()*0.6);
		var top= (area && area.top) ? area.top : (($(window).height()-height)/2);
		var title = fox.translate('机柜');
		...
		admin.popupCenter({
			...
			area: ["80%",height+"px"],
			type: 2,
			id:"eam-asset-rack-info-form-data-win",
			content: '/business/datacenter/asset_rack_info/asset_rack_info_form.html' + (queryString?("?"+queryString):""),
			finish: function () {
				if(action=="create") {
					refreshTableData();
				}
