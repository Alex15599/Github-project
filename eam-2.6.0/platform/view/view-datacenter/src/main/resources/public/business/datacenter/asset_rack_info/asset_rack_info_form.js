...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('eam-asset-rack-info-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('eam-asset-rack-info-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'eam-asset-rack-info-form-data-win');
			if(area==null) return;
			admin.putTempData('eam-asset-rack-info-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("rackType",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("rackUsedType",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			transform: function(data) {
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('eam-asset-rack-info-form-data');
		}
		...
		if (hasData) {
			...
			//设置  类型 设置下拉框勾选
			fox.setSelectValue4QueryApi("#rackType",formData.typeDict);
			//设置  状态 设置下拉框勾选
			fox.setSelectValue4QueryApi("#status",formData.statusDict);
			//设置  环境 设置下拉框勾选
			fox.setSelectValue4QueryApi("#environment",formData.environmentDict);
			//设置  使用分类 设置下拉框勾选
			fox.setSelectValue4QueryApi("#rackUsedType",formData.usedTypeDict);
	...
	function getFormData() {
		...
		//获取 类型 下拉框的值
		data["rackType"]=fox.getSelectedValue("rackType",false);
		//获取 状态 下拉框的值
		data["status"]=fox.getSelectedValue("status",false);
		//获取 环境 下拉框的值
		data["environment"]=fox.getSelectedValue("environment",false);
		//获取 使用分类 下拉框的值
		data["rackUsedType"]=fox.getSelectedValue("rackUsedType",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('eam-asset-rack-info-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('eam-asset-rack-info-form-data-win',this); });
