...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('dc-layer-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('dc-layer-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'dc-layer-form-data-win');
			if(area==null) return;
			admin.putTempData('dc-layer-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("areaId",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('dc-layer-form-data');
		}
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('dc-layer-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('dc-layer-form-data-win',this); });
