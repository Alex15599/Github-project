...
function FormPage() {
	...
	this.init=function(layui) {
		...
		action=admin.getTempData('hr-certificate-form-data-form-action');
		//如果没有修改和保存权限
		...
		if(window.pageExt.form.beforeInit) {
			window.pageExt.form.beforeInit(action,admin.getTempData('hr-certificate-form-data'));
		}
	...
	function adjustPopup() {
		...
		adjustPopupTask=setTimeout(function () {
			...
			var area=admin.changePopupArea(null,bodyHeight+footerHeight,'hr-certificate-form-data-win');
			if(area==null) return;
			admin.putTempData('hr-certificate-form-area', area);
	...
	function renderFormFields() {
		...
		fox.renderSelectBox({
			...
			on: function(data){
				setTimeout(function () {
					window.pageExt.form.onSelectBoxChanged && window.pageExt.form.onSelectBoxChanged("certLevel",data.arr,data.change,data.isAdd);
				},1);
			},
			//转换数据
			searchField: "name", //请自行调整用于搜索的字段名称
	...
	function fillFormData(formData) {
		if(!formData) {
			formData = admin.getTempData('hr-certificate-form-data');
		}
		...
		if (hasData) {
			...
			//设置  类型 设置下拉框勾选
			fox.setSelectValue4QueryApi("#type",formData.certificateType);
			//设置  级别 设置下拉框勾选
			fox.setSelectValue4QueryApi("#certLevel",formData.certificateLevel);
	...
	function getFormData() {
		...
		//获取 级别 下拉框的值
		data["certLevel"]=fox.getSelectedValue("certLevel",false);
	...
	function saveForm(param,callback) {
		...
		admin.post(api, param, function (data) {
			if (data.success) {
				...
				if(doNext) {
					admin.finishPopupCenterById('hr-certificate-form-data-win');
				}
    ...
    function bindButtonEvent() {
	    ...
	    //关闭窗口
	    $("#cancel-button").click(function(){ admin.finishPopupCenterById('hr-certificate-form-data-win',this); });
