...
public class SafetylevelServiceImpl extends SuperService<Safetylevel> implements ISafetylevelService {
	...
	public Result deleteByIdPhysical(String id) {
		...
		safetylevel.setId(id);
		try {
			boolean suc = dao.deleteEntity(safetylevel);
	...
	public Result deleteByIdLogical(String id) {
		...
		if(id==null) return ErrorDesc.failure().message("id 不允许为 null 。");
		safetylevel.setId(id);
		safetylevel.setDeleted(dao.getDBTreaty().getTrueValue());
		safetylevel.setDeleteBy((String)dao.getDBTreaty().getLoginUserId());
		safetylevel.setDeleteTime(new Date());
		try {
			boolean suc = dao.updateEntity(safetylevel,SaveMode.NOT_NULL_FIELDS);
			return suc?ErrorDesc.success():ErrorDesc.failure();
		}
		catch(Exception e) {
			Result r= ErrorDesc.failure();
	 ...
	 *
	 * @param id 主键
	 * @return Safetylevel 数据对象
	 */
	public Safetylevel getById(String id) {
		Safetylevel sample = new Safetylevel();
		if(id==null) throw new IllegalArgumentException("id 不允许为 null ");
		sample.setId(id);
	 ...
	 * @return 判断结果
	 */
	public Result<Safetylevel> checkExists(Safetylevel safetylevel) {
		//TDOD 此处添加判断段的代码
		//boolean exists=this.checkExists(safetylevel, SYS_ROLE.NAME);
		//return exists;
		return ErrorDesc.success();
	}

	@Override
	public ExcelWriter exportExcel(Safetylevel sample) {
