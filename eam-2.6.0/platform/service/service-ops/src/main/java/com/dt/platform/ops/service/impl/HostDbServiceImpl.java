...
public class HostDbServiceImpl extends SuperService<HostDb> implements IHostDbService {
	...
	public Result deleteByIdPhysical(String id) {
		...
		hostDb.setId(id);
		try {
			boolean suc = dao.deleteEntity(hostDb);
	...
	public Result deleteByIdLogical(String id) {
		...
		if(id==null) return ErrorDesc.failure().message("id 不允许为 null 。");
		hostDb.setId(id);
		hostDb.setDeleted(dao.getDBTreaty().getTrueValue());
		hostDb.setDeleteBy((String)dao.getDBTreaty().getLoginUserId());
		hostDb.setDeleteTime(new Date());
		try {
			boolean suc = dao.updateEntity(hostDb,SaveMode.NOT_NULL_FIELDS);
			return suc?ErrorDesc.success():ErrorDesc.failure();
		}
		catch(Exception e) {
			Result r= ErrorDesc.failure();
	 ...
	 * @return HostDb 数据对象
	 */
	public HostDb getById(String id) {
		HostDb sample = new HostDb();
		if(id==null) throw new IllegalArgumentException("id 不允许为 null ");
		sample.setId(id);
	 ...
	 * @return 判断结果
	 */
	public Result<HostDb> checkExists(HostDb hostDb) {
		//TDOD 此处添加判断段的代码
		//boolean exists=this.checkExists(hostDb, SYS_ROLE.NAME);
		//return exists;
		return ErrorDesc.success();
	}

	@Override
	public ExcelWriter exportExcel(HostDb sample) {
...
/**
	...
	public void saveRelation(String hostId,List<String> serviceInfoIds) {
		super.saveRelation(HostDb.class,OPS_HOST_DB.HOST_ID,hostId, ServiceInfo.class,OPS_HOST_DB.SERVICE_INFO_ID,serviceInfoIds,true);
	}
