...
public class RackController extends SuperController {
    ...
    public Result<Rack> getById(String id) {
        ...
        // join 关联的对象
        rackService.dao().fill(rack).with(RackMeta.STATUS_DICT).with(RackMeta.TYPE_DICT).with(RackMeta.USED_TYPE_DICT).with(RackMeta.ENVIRONMENT_DICT).with(RackMeta.AREA).with(RackMeta.LAYER).execute();
    ...
    public Result<PagedList<Rack>> queryPagedList(RackVO sample) {
        ...
        // join 关联的对象
        rackService.dao().fill(list).with(RackMeta.STATUS_DICT).with(RackMeta.TYPE_DICT).with(RackMeta.USED_TYPE_DICT).with(RackMeta.ENVIRONMENT_DICT).with(RackMeta.AREA).with(RackMeta.LAYER).execute();
        result.success(true).data(list);
        return result;
    }
