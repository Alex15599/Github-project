...
public class ServerApplyBpmEventAdaptor extends BpmEventAdaptor<ServerApply,IServerApplyService> {


	public String BPM_TABLE="wo_server_apply";

	private void updateBillStatus(String id,String status ){
		this.dao().execute("update "+BPM_TABLE+" set status=? where id=?", status, id);
	}

	public ServerApplyBpmEventAdaptor(IServerApplyService service) {
		super(service);
	}
